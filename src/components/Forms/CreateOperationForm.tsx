...
export const CreateOperationForm: FC<CreateOperationForm> = ({ token, closeFunc }) => {
  ...
  const {
    ...
    data: categories,
	} = useAppSelector((state) => state.categoryReduser);
	  ...
	  <>
		...
		{categories.data.length ?
				...
				>
					{({ errors, touched }) => (
						<Form className={cl.form}>
							...
							<label className={cl.label}>
								<span>Описание </span>
								<Field className={cl.input} placeholder="operationDescription" type="textfield" name="desc" validate={validateField} />
				...
				<div className={cl.noCategpriesError}>
					...
					<Button
						onClick={() => createCategoryWithRedirect()}
					>
						Создать категорию
					</Button>
				</div>
